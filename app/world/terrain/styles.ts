...
export const STYLES: Record<string, TerrainStyle> = {
  default: {
    name: 'default',
    label: 'Highlands',
    shape: { roughness: 0.45, ridged: 0.35, warp: 0.25, erosion: 12000, curve: 1.35, maxHeight: 70 },
    palette: {
      low: '#6f6a4f',
      mid: '#3f6b4a',
      high: '#6b7a8f',
  ...
  icy: {
    name: 'icy',
    label: 'Icy Peaks',
    // Sharp and tall: high ridged weight makes knife-edge aretes, heavy erosion
    // cuts the glacial valleys between them, low snowline puts white everywhere.
    shape: { roughness: 0.62, ridged: 0.85, warp: 0.2, erosion: 18000, curve: 1.55, maxHeight: 95 },
    palette: {
      low: '#5b6a78',
      mid: '#8fa3b5',
      high: '#c3d2e0',
  ...
  blossom: {
    ...
    shape: { roughness: 0.28, ridged: 0.1, warp: 0.45, erosion: 7000, curve: 0.95, maxHeight: 42 },
    palette: {
      low: '#8f7f6a',
      mid: '#7fa86a',
      high: '#c98fa8',
  ...
  volcanic: {
    ...
    shape: { roughness: 0.55, ridged: 0.5, warp: 0.15, erosion: 4000, curve: 1.9, maxHeight: 88 },
    palette: {
      low: '#3a3230',
      mid: '#2e2724',
      high: '#4a3a34',
  ...
  desert: {
    name: 'desert',
    label: 'Desert Mesa',
    // Flat tops: curve below 1 plus low roughness gives plateaus, and near-zero
    // erosion keeps the edges square instead of rounding them off.
    shape: { roughness: 0.2, ridged: 0.15, warp: 0.6, erosion: 2000, curve: 0.6, maxHeight: 55 },
    palette: {
      low: '#c9a227',
      mid: '#b5763a',
      high: '#8f4f2f',
  ...
  verdant: {
    ...
    // Deep erosion is the point here — lots of droplets carve the branching
    // river valleys that make lowland terrain read as lush rather than empty.
    shape: { roughness: 0.38, ridged: 0.25, warp: 0.35, erosion: 22000, curve: 1.15, maxHeight: 60 },
    palette: {
      low: '#4a6b3a',
      mid: '#2f5c34',
      high: '#5f6b4a',
